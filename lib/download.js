...
function resolveMavenVersion(libDir, groupId, artifactId, version, downloadExpirationTimeMs) {
    if(version && version !== 'latest') {
        ...
        const xmlReqeust = saveCachedUrlToPath(latestCacheFile, `https://maven-central-asia.storage-download.googleapis.com/maven2/${groupId.replace(/\./g, '/')}/${artifactId}/maven-metadata.xml`, downloadExpirationTimeMs);
    ...
    return resolveMavenVersion(libDir, groupId, artifactId, version, downloadExpirationTimeMs)
        .then(function(version) {
            if(version.match(/^https/)) {
                ...
                const flywayUrl = `https://maven-central-asia.storage-download.googleapis.com/maven2/${groupId.replace(/\./g, '/')}/${artifactId}/${version}/${artifactId}-${version}-${platformSuffix}`;
                ...
                const depUrl = `https://maven-central-asia.storage-download.googleapis.com/maven2/${groupId.replace(/\./g, '/')}/${artifactId}/${version}/${artifactId}-${version}.jar`;
